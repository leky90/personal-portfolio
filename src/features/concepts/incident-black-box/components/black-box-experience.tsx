...
const PHASES: Phase[] = [
  {
    ...
    copy: "Connection pool bắt đầu trôi lúc 14:11. Pager chỉ kêu lúc 14:27. Băng ghi lại sự thật khó chịu: tín hiệu luôn đến trước cảnh báo 16 phút, chỉ là chưa ai đặt ngưỡng cho nó.",
    side: "left",
  },
  {
    title: "Cầm máu trước, chẩn đoán sau",
    copy: "14:41, quyết định quan trọng nhất của ca trực: shed 20% read traffic TRƯỚC khi biết root cause. Error rate gãy xuống trong 90 giây, mua đủ thời gian để tìm ra retry storm.",
    side: "right",
  },
  {
    title: "Một dòng config, hai người review",
    copy: "Root cause là client library tự retry 5 lần khi pool timeout, tự khuếch đại chính nó. Bản vá: cap retry + jitter. Một dòng, review giữa sự cố, băng nguội dần từ 14:58.",
    side: "left",
  },
];
...
const LESSONS: Lesson[] = [
  {
    ...
    copy: "Alert đến sau tín hiệu 16 phút. Ngưỡng tốt canh trên đạo hàm, không phải trên giá trị.",
  },
  {
    id: "mitigate-first",
    title: "Mitigate trước root cause",
    copy: "Shed traffic mua 13 phút suy nghĩ. Hệ thống sống sót nhờ thứ tự ưu tiên, không nhờ thiên tài.",
  },
  {
    id: "retry-budget",
    title: "Retry cần ngân sách",
    copy: "Cap + jitter là một dòng config. Học phí của dòng đó là 43 phút SEV-1.",
  },
];
    ...
    <div ref={containerRef} className="relative">
      ...
      <section className="mx-auto flex min-h-dvh w-full max-w-5xl flex-col justify-center px-4 pt-20 sm:px-6">
        <p className="font-mono text-[11px] tracking-[0.3em] text-[#ff5a1f] uppercase">
          Blameless postmortem · một sự cố thật đã khử danh
        </p>
        ...
        <p className="mt-4 font-mono text-xs text-neutral-400">
          {INCIDENT.sev} · {INCIDENT.durationMin} phút · MTTR{" "}
          {INCIDENT.mttrMin} phút · {INCIDENT.date}
        </p>
        <p className="mt-5 max-w-md text-sm leading-relaxed text-neutral-400">
          Hộp đen máy bay sơn cam vì để được tìm thấy. Băng telemetry phía sau
          cũng vậy: cuộn để tua {INCIDENT.durationMin} phút của một SEV-1 qua
      ...
      <section className="mx-auto flex min-h-dvh w-full max-w-5xl flex-col justify-center px-4 sm:px-6">
        ...
        <div className="mt-12 w-full max-w-2xl rounded-lg border border-neutral-900 bg-neutral-950/70 p-5">
          ...
          <p className="mt-3 text-sm leading-relaxed text-neutral-400">
            ...
            Đây chính là loại decision artifact mà hiring guide 2026 yêu cầu
            senior trưng ra.
          </p>
        </div>
      </section>
