...
describe("prism-data — 5 ràng buộc gập một tia ý tưởng thành thiết kế", () => {
  ...
  it("activeDecisions: quyết định chỉ bật khi ĐỦ ràng buộc nó cần", () => {
    ...
    const pciIndex = CONSTRAINTS.findIndex((c) => c.id === "pci");
    const onlyPci = CONSTRAINTS.map((_, i) => i === pciIndex);
    const ids = activeDecisions(onlyPci).map((d) => d.id);
    expect(ids).toContain("audit-log");
    expect(ids).not.toContain("boring-postgres");
  });
