 ...
 * death trên timeline 2014→2026, mọi animation là hàm thuần của MỘT
 * giá trị progress (scrub-safe, demand-safe). Dữ liệu bám đúng bốn
 * chặng nghề thật: freelance PHP ở Huế (2014), văn phòng Synova ở
...
export const DESK_OBJECTS: DeskObject[] = [
  {
    ...
    story: "Mặt bàn đầu tiên, 2014, trong một căn phòng ở Huế — dựng ngay sau khi tốt nghiệp ĐH Khoa học Huế. Mọi thứ khác đến rồi đi; riêng mặt bàn chỉ được refactor chứ chưa lần nào bị thay.",
    birth: 0,
    death: 1,
    kind: "box",
    material: "wood",
  ...
  },
  {
    ...
    story: "Cỗ máy độc nhất suốt thời freelance PHP từ 2014 — cũ sẵn từ hồi sinh viên nhưng vẫn chạy tốt, và mọi job đầu đời đều chạy trên đúng cái máy này (tài khoản Freelancer thì mở sẵn từ 2012, thời còn đi học). Nó chỉ nghỉ khi tôi chuyển hẳn sang stack JavaScript.",
    birth: 0,
    death: 0.44,
    kind: "box",
    material: "plastic",
...
export const ERAS: DeskEra[] = [
  {
    ...
    t: 0,
    accent: "#d97b53",
    note: "Tốt nghiệp ĐH Khoa học Huế rồi freelance tại chỗ: PHP, WordPress, HTML/CSS/JS, lo cross-browser và responsive cho khách. Một laptop, một chồng sách, và mọi commit đều là thêm vào.",
  },
  {
    year: 2017,
    t: 0.25,
...
/** Số năm timeline phủ: 2014 (tốt nghiệp, bắt đầu đi làm) → 2026 (hôm nay). */
const YEAR_SPAN = 12;

/** Năm hiển thị trên ticker: 0 → 2014, 1 → 2026. */
export function yearAt(t: number): number {
  return 2014 + Math.round(Math.min(Math.max(t, 0), 1) * YEAR_SPAN);
}
