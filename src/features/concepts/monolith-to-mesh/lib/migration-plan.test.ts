...
describe("migration plan — lịch sử tách service 10 năm", () => {
  ...
  it("core (không tách) ≥ 4 service; năm tách trong 2017..2025", () => {
    const core = SERVICES.filter((s) => s.splitYear === null);
    expect(core.length).toBeGreaterThanOrEqual(4);
    for (const s of SERVICES) {
      if (s.splitYear !== null) {
        expect(s.splitYear).toBeGreaterThanOrEqual(2017);
