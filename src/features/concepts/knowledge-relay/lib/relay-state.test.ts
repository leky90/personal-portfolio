...
describe("relay-state — cầu nối DOM ↔ canvas", () => {
  it("mặc định: năm 2012, chưa chọn baton, chưa gắn invalidate", () => {
  ...
  it("relayYearFromProgress map [0,1] → [2012,2026] có clamp", () => {
    expect(relayYearFromProgress(0)).toBe(2012);
    expect(relayYearFromProgress(0.5)).toBe(2019);
    expect(relayYearFromProgress(1)).toBe(2026);
    expect(relayYearFromProgress(-1)).toBe(2012);
